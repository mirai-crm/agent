...
Для каждого ФОП терминала задаётся соответствие TIN → индекс мерчанта:
...
Индекс берётся из `getMaskList` терминала. Повторный `setup` для уже
настроенного устройства сохраняет существующие соответствия.
...
- `amountMinor` — обязательное положительное целое число копеек:
  `12345` = `123.45 UAH`.
- `tin` — обязательная непустая строка с ЄДРПОУ/ДРФО ФОП. Агент находит
  `merchantId` в `devices.pos.merchant_ids`.
- Неизвестные поля в `data`, дробная сумма и дополнительные JSON-значения не
  допускаются.
- Пустой или неизвестный `tin` завершает задачу через `error_message` без
  записи в платёжный журнал и без обращения к терминалу.
...
`amountMinor` и `tin` — нормализованный исходный запрос. Фактически
